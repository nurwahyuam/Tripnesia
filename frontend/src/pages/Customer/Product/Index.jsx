// src/pages/Customer/Product/Index.jsx
...
const Index = () => {
  ...
  // State untuk melacak apakah ada pending booking di antara semua ship
  ...
  // Effect untuk mengecek apakah ada pending booking di antara semua ship
  useEffect(() => {
    const checkAllShipsForPendingBooking = () => {
      let hasPending = false;

      // Cek semua hasil active booking
  ...
  const filteredTrips = useMemo(() => {
    return mappedTrips.filter((trip) => {
      ...
      if (filters.classTrip.length > 0) {
        ...
        if (!filters.classTrip.includes(tripClass)) return false;
      }

      if (filters.shipBrand.length > 0 && !filters.shipBrand.includes(trip.brand)) return false;

      if (filters.budget.length > 0) {
        if (!trip.minPrice || trip.minPrice === 0) return false;
  ...
  // Perbarui effect untuk menggunakan hook yang baru
  useEffect(() => {
    const tripsToCheck = sortedTrips.slice(0, visibleCount);
    tripsToCheck.forEach((trip) => {
      if (activeCheckResult[trip.shipId] === undefined) {
        checkActiveBooking(trip.shipId); // Gunakan fungsi baru
      }
    });
  }, [sortedTrips, visibleCount, checkActiveBooking, activeCheckResult]); // Gunakan dependency baru

  const displayedTrips = sortedTrips.slice(0, visibleCount);

  // Fungsi untuk menentukan apakah card harus non-active
  const shouldDisableCard = (trip) => {
    ...
    const activeCheck = activeCheckResult[trip.shipId];
                ...
                {sortedTrips.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {displayedTrips.map((trip) => {
                      const isCardDisabled = shouldDisableCard(trip);
                      const statusMessage = getStatusMessage(trip);
                      const activeCheck = activeCheckResult[trip.shipId];
                      const isPendingCheckLoading = activeCheckLoading[trip.shipId];

                      return (
                        <div className="relative" key={trip.id}>
                          <div className="absolute top-1.5 right-1.5 z-10">
                            <FavoriteButton shipId={trip.shipId} size={20} className="shadow-md" />
                          </div>
                          <Link
                            to={activeCheck?.hasActiveBooking ? `/customer/order/${activeCheck.bookingId}` : `/customer/product/${trip.slug}`}
                            className={`bg-white rounded-2xl border border-gray-300 hover:shadow-sm transition-shadow duration-300 flex flex-col ${!trip.hasCabins || isCardDisabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
                            onClick={(e) => {
                              if (!trip.hasCabins || isCardDisabled) {
                                e.preventDefault();
                              }
                            }}
                          >
                            ...
                            {/* Tampilkan loading jika sedang mengecek */}
                  ...
                  <>
                      ...
                      <div className="relative my-12">
                        ...
                        <div className="relative z-10 text-center">
                          <button
                            ...
                            disabled={hasAnyPendingBooking}
                            className={`inline-block bg-white px-4 text-md font-semibold border border-white py-2 ${hasAnyPendingBooking ? "text-gray-400 cursor-not-allowed" : "text-gray-800 hover:cursor-pointer"}`}
                          >
                            <span className={`border px-20 py-3 rounded-4xl ${hasAnyPendingBooking ? "border-gray-300" : "border-gray-400"}`}>Show More</span>
                          </button>
                        </div>
                      </div>
                    )}
                    ...
                    {sortedTrips.length <= visibleCount && (
                      <div className="relative my-12">
                        <div className="absolute top-1/2 left-0 right-0 h-px bg-gray-400"></div>
                        <div className="relative z-10 text-center">
                          <div className="inline-block bg-white px-4 text-md font-semibold text-gray-400 border border-white py-2 hover:cursor-not-allowed">
