...
const Detail = () => {
  ...
  const { getBookedPax } = useConfirmedBookings(ship?._id);

  console.log(getBookedPax);

  const updatePax = (cabinId, type, change) => {
    const current = selectedCabins[cabinId] || { adult: 0, child: 0 };
    const cabin = ship.cabins.find((c) => c._id === cabinId);
    if (!cabin) return;

    if (type === "adult") {
      const bookedPax = selectedRange ? getBookedPax(cabinId) : 0;
      const maxAvailable = cabin.pax - bookedPax;
      const newValue = Math.max(0, current.adult + change);
      if (newValue > maxAvailable) return;

      setSelectedCabins({
        ...selectedCabins,
        [cabinId]: { ...current, adult: newValue },
      });
    }
    ...
    if (type === "child") {
      ...
      setSelectedCabins({
        ...selectedCabins,
        [cabinId]: { ...current, child: newValue },
      });
    }
  };

  // Di dalam komponen Detail
  const isBookingValid = () => {
    // Cek apakah ada kabin dengan pax > 0
    const hasSelection = Object.values(selectedCabins).some((pax) => (pax.adult || 0) + (pax.child || 0) > 0);

    if (!hasSelection) return false;

    // Cek setiap kabin yang dipilih: apakah melebihi kapasitas tersedia?
    for (const cabinId in selectedCabins) {
      const pax = selectedCabins[cabinId];
      const totalSelected = (pax.adult || 0) + (pax.child || 0);
      ...
      const bookedPax = selectedRange ? getBookedPax(cabinId) : 0;
      const availablePax = cabin.pax - bookedPax;

      // Jika pax yang dipilih > kapasitas tersisa → invalid
      if (pax.adult > availablePax) {
        return false;
      }
    }
  ...
  const availableRanges = ship?.cabins ? generateUniqueDateRanges(ship.cabins) : [];
  ...
  const filteredCabins = ship?.cabins ? ship.cabins.filter((cabin) => isCabinInSelectedRange(cabin, selectedRange)) : [];
  ...
  const handleBooking = () => {
    ...
    navigation(`/customer/product/${ship.slug}/checkout`, { state: { bookingData } });
  };

  return (
    <CustomerLayout hideNavbar={!isScrolled ? false : true}>
      <Breadcrumb />
      <div className="container mx-auto px-4 py-8">
        {/* Back Button */}
        <Link to={"/customer/product"} className="flex items-center gap-2 mb-8 text-sm">
          <ArrowLeft size={18} />
          See all ships
        </Link>
          ...
          <section id="about-ship" className="scroll-mt-16 mt-4">
            ...
            <div className="flex">
              ...
              <div className="w-1/4 border border-gray-300 p-5 rounded-xl">
                ...
                <button onClick={() => scrollToSection("cabins")} className="w-full bg-primary hover:opacity-60 text-white py-2 rounded-lg transition-all font-medium mb-4">
                  Select Date
                </button>
                ...
                {filteredCabins.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredCabins.map((cabin) => {
                      ...
                      // 🔥 Ambil jumlah pax yang sudah dibooking (confirmed)
                      const bookedPax = getBookedPax(cabin._id); // ← tanpa tanggal
                      const availablePax = Math.max(0, cabin.pax - bookedPax);
                      const isFullyBooked = availablePax <= 0;
                      const canAddAdult = currentPax.adult < availablePax && !isFullyBooked;

                      return (
                        <div key={cabin._id} className={`border border-gray-300 rounded-xl overflow-hidden relative ${isFullyBooked ? "opacity-60" : ""}`}>
                          {isFullyBooked && (
                            <div className="absolute inset-0 bg-white bg-opacity-70">
                              ...
                              <div className="p-4">
                                ...
                                <div className="text-xs text-red-500 mb-2">
                                  <span>Cabin is Full</span>
                                </div>

                                {/* Tampilkan selector hanya jika ada rentang & tidak penuh */}
                                {selectedRange && !isFullyBooked ? (
                                      ...
                                      <div className="mt-2">
                                        ...
                                        <div className="flex items-center justify-center gap-2 mb-6 border border-gray-300 py-1.5 px-1.5 rounded-xl">
                                          ...
                                          <button
                                            ...
                                            className={`w-8 h-8 flex border border-gray-300 rounded-lg items-center justify-center ${!canAddAdult ? "cursor-not-allowed bg-gray-200" : "hover:bg-gray-300"}`}
                                          >
                                        ...
                                        <div className="flex items-center gap-2 border border-gray-300 py-1.5 justify-center rounded-xl">
                                          ...
                                          <button
                                            ...
                                            className={`w-8 h-8 flex items-center justify-center rounded-lg border border-gray-300 ${currentPax.child >= 2 ? "bg-gray-200 cursor-not-allowed" : "hover:bg-gray-300"}`}
                                          >
                                            <Plus size={14} className="text-gray-800" />
                                          </button>
                                        </div>
                          ...
                          <div className="p-4">
                            ...
                            <div className="text-xs text-gray-500 mb-2">
                              {bookedPax > 0 ? (
                                <span>
                                  {availablePax} of {cabin.pax} pax available
                                </span>
                              ) : (
                                <span>{cabin.pax} pax capacity</span>
                              )}
                            </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-xl p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto z-51">
            ...
            <div className="mt-3 flex-col space-y-1.5">
              ...
              <button onClick={handleBooking} className="w-full px-4 py-2 bg-primary text-white rounded-xl hover:opacity-60 transition-all" disabled={!isBookingValid()}>
                Continue
              </button>
            </div>
          </div>
