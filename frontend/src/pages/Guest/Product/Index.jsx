...
const Index = () => {
  ...
  const filterOptions = {
    ...
    budget: ["Less than IDR 2,000,000", "IDR 2,000,000 - IDR 5,000,000", "IDR 5,000,000 - IDR 7,000,000", "More than IDR 10,000,000"],
    shipBrand: ["Sarba Samuda Marine", "Andatama", "Oeds", "Palindo Marine", "Printed Nusantara"],
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="space-y-3">
                  {filterOptions.tripDuration.map((duration) => (
                    <label key={duration} className="flex items-center space-x-3 cursor-pointer">
                      <input type="checkbox" checked={filters.tripDuration.includes(duration)} onChange={() => handleFilterChange("tripDuration", duration)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                ...
                <div className="space-y-3">
                  {filterOptions.classTrip.map((classType) => (
                    <label key={classType} className="flex items-center space-x-3 cursor-pointer">
                      <input type="checkbox" checked={filters.classTrip.includes(classType)} onChange={() => handleFilterChange("classTrip", classType)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                ...
                <div className="space-y-3">
                  {filterOptions.budget.map((budget) => (
                    <label key={budget} className="flex items-center space-x-3 cursor-pointer">
                      <input type="checkbox" checked={filters.budget.includes(budget)} onChange={() => handleFilterChange("budget", budget)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                ...
                <div className="space-y-3">
                  {filterOptions.shipBrand.map((brand) => (
                    <label key={brand} className="flex items-center space-x-3 cursor-pointer">
                      <input type="checkbox" checked={filters.shipBrand.includes(brand)} onChange={() => handleFilterChange("shipBrand", brand)} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
          ...
          <div className="lg:col-span-3">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Book a Boat Rental in Raja Ampat</h1>

              {/* Trip Type Selector */}
              <div className="flex space-x-4 mb-6">
                <button className={`px-6 py-3 rounded-lg font-medium transition-colors ${tripType === "Open Trip" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`} onClick={() => setTripType("Open Trip")}>
                  Open Trip
                </button>
                <button className={`px-6 py-3 rounded-lg font-medium transition-colors ${tripType === "Private Trip" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`} onClick={() => setTripType("Private Trip")}>
                  Private Trip
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Select trip start date</label>
                  <input type="date" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Select trip end date</label>
                  <input type="date" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-gray-800">{filteredTrips.length} results found</h2>
                ...
                <div key={trip.id} className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-shadow duration-300">
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      <span className="px-2 py-1 bg-blue-100 text-blue-600 text-xs font-medium rounded">{trip.type}</span>
                      <span className="px-2 py-1 bg-green-100 text-green-600 text-xs font-medium rounded">{trip.duration}</span>
                    </div>

                    <h3 className="font-semibold text-gray-800 mb-2 line-clamp-2">{trip.title}</h3>
                    ...
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded">{trip.class}</span>
                    </div>

                    <button className="w-full py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">Book Now</button>
                  </div>
                </div>
              ))}
            </div>
