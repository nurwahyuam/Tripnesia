...
const TripSlider = ({ trips }) => {
  ...
  const tripsWithCabins = trips?.filter((trip) => trip.cabins && Array.isArray(trip.cabins) && trip.cabins.length > 0) || [];

  const shouldShowChevrons = tripsWithCabins.length > 4;
  ...
  return (
    <div className="relative h-125">
      ...
      <div ref={sliderRef} className="trip-slider flex gap-8 py-4" style={{ scrollBehavior: "smooth" }}>
        {tripsWithCabins.map((trip, index) => (
